    ...
    return (
        <div className="flex gap-1">
            {Array.from({ length: 5 }, (_, i) => (
                <Star
                    key={i}
                    className={`h-4 w-4 ${i < rating ? 'fill-[#C9A84C] text-[#C9A84C]' : 'fill-white/5 text-white/10'}`}
                />
            ))}
        </div>
    );
...
export default function TestimonialsSection({ testimoni }: Props) {
    ...
    return (
        <section id="testimoni" ref={sectionRef} className="relative overflow-hidden bg-[#0D1B2A] py-28 lg:py-36">
            ...
            <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <div className={`mb-16 text-center transition-all duration-700 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                    <div className="mb-4 flex items-center justify-center gap-3">
                        ...
                        <span className="text-xs font-bold tracking-[0.3em] text-[#C9A84C] uppercase">Testimoni</span>
                        <div className="h-px w-12 bg-[#C9A84C]" />
                    </div>
                    <h2 className="text-4xl font-black leading-tight tracking-tight text-white lg:text-6xl uppercase">
                        Apa Kata
                        <br />
                        <span className="text-[#C9A84C]">Klien Kami</span>
                    </h2>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {data.map((item, i) => (
                        <div
                            key={item.id}
                            className={`group relative flex flex-col overflow-hidden rounded-2xl border border-white/6 bg-white/2 transition-all duration-500 hover:border-[#C9A84C]/20 hover:bg-white/4 ${
                                visible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                            }`}
                            style={{ transitionDelay: `${200 + i * 120}ms` }}
                        >
                            {/* Gold top accent */}
                            <div className="h-[2px] w-full bg-linear-to-r from-transparent via-[#C9A84C]/50 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />

                            <div className="flex flex-1 flex-col p-8">
                                {/* Top: stars + quote icon */}
                                <div className="mb-6 flex items-center justify-between">
                                    <Stars rating={item.rating} />
                                    <Quote className="h-8 w-8 text-[#C9A84C]/12 group-hover:text-[#C9A84C]/25 transition-colors" />
                                </div>
                            ...
                            <div className="flex items-center gap-4 border-t border-white/5 bg-white/[0.015] px-8 py-5">
                                ...
                                <div className="relative shrink-0">
                                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-linear-to-br from-[#C9A84C]/25 to-[#C9A84C]/5 text-sm font-black text-[#C9A84C]">
                                        {item.nama.charAt(0)}
                                    </div>
                                    <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[#0D1B2A] bg-[#C9A84C]/60" />
                                </div>
                                <div className="min-w-0">
                                    ...
                                    <p className="truncate text-[10px] font-bold tracking-wider text-[#C9A84C]/50 uppercase">{item.perusahaan}</p>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <div ref={quoteBannerRef} className={`relative mt-20 min-h-[280px] overflow-hidden transition-all duration-700 delay-500 sm:min-h-[320px] lg:min-h-[380px] ${visible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                ...
                <div className="absolute inset-0 bg-linear-to-r from-[#0D1B2A]/90 via-[#0D1B2A]/60 to-[#0D1B2A]/80" />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center px-6 py-16 text-center lg:px-20">
                    <Quote className="mb-6 h-10 w-10 text-[#C9A84C]/30 md:h-12 md:w-12" />
                    <p className="mx-auto max-w-3xl text-lg font-black leading-relaxed tracking-tight text-white md:text-xl lg:text-3xl uppercase">
                        ...
                        <span className="text-[#C9A84C]">ini adalah komitmen kami untuk melindungi yang Anda percayakan.</span>
                    </p>
                    <div className="mt-6 flex items-center gap-3 md:mt-8">
                        <div className="h-px w-8 bg-[#C9A84C]/40 md:w-12" />
                        <span className="text-[10px] font-bold tracking-[0.3em] text-[#C9A84C]/60 uppercase">Tribuana Security</span>
                        <div className="h-px w-8 bg-[#C9A84C]/40 md:w-12" />
                    </div>
                </div>
            </div>
