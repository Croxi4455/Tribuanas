...
export default function GallerySection({ galeri }: Props) {
    ...
    return (
        <section id="galeri" ref={sectionRef} className="relative overflow-hidden bg-[#0A0A0A] py-28 lg:py-36">
            ...
            <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <div className={`mb-12 transition-all duration-700 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-xs font-bold tracking-[0.3em] text-[#C9A84C] uppercase">Galeri</span>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {filtered.map((item, i) => (
                        <div
                            key={item.id}
                            className={`group relative aspect-[4/3] overflow-hidden rounded-xl border border-white/5 bg-[#111827] transition-all duration-500 hover:border-[#C9A84C]/20 hover:shadow-xl hover:shadow-[#C9A84C]/10 ${
                        ...
                        >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 translate-y-full bg-linear-to-t from-[#0D1B2A] to-transparent p-4 transition-transform duration-300 group-hover:translate-y-0">
                                ...
                                <span className="mt-1 inline-block rounded-full border border-[#C9A84C]/20 bg-[#C9A84C]/10 px-2.5 py-0.5 text-[10px] font-bold capitalize tracking-widest text-[#C9A84C] uppercase">
                                    {item.kategori}
                                </span>
                            </div>
                        </div>
